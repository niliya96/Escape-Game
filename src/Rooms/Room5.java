...
public class Room5 extends JPanel implements Room {
	...
	String kettleS = "/kettle_sound.wav";
	...
	class MyMouseListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent e) {
			 ...
			 * inventory items
			 */
			if (spray.ifChose(e) && !ifSprayDeleted && !ifWantToExit) {
				ifSprayChoose = !ifSprayChoose;
				ifCableChoose = false;
				ifDiskChoose = false;
			}
			if (cable.ifChose(e) && ifCablePicked && !ifCableDeleted && !ifWantToExit) {
				ifCableChoose = !ifCableChoose;
				ifSprayChoose = false;
				ifDiskChoose = false;
			}
			if (disk.ifChose(e) && ifDiskPicked && !ifDiskDeleted && !ifWantToExit) {
			...
			if (!ifCloseFrame) {
				...
				if (buttonPassword.ifClicked(e) && ifDiskOnKey && !ifCodeComputer && !ifWantToExit) {
				...
				if (ifComputerClose && !ifWantToExit) {
					if (ifDiskChoose && computer.ifClicked(e)) {
						ifDiskOnKey = true;
					}
				}
				if (ifDiskOnKey && !ifWantToExit) {
					if (ifComputerClose) {
				...
				if (computer.ifClicked(e) && !picture.ifClicked(e) && !ifDrawer1 && !ifDrawer2 && !ifWantToExit) {
				...
				// close wall
				if (e.getX() >= 1000 && e.getX() <= 1200 && e.getY() >= 660 && e.getY() <= 755 && !ifWantToExit) {
					ifCloseFrame = true;
					ifCloseWall = true;
				}
				// middle drawer
				if (e.getX() >= 1220 && e.getX() <= 1380 && e.getY() >= 620 && e.getY() <= 695 && !ifDrawer2
						&& !ifPictureClose && !ifComputerClose && !ifWantToExit) {
					ifDrawer1 = true;
				}
				if (ifDrawer1 && !ifWantToExit) {
				...
				if (e.getX() >= 920 && e.getX() <= 1220 && e.getY() >= 565 && e.getY() <= 620 && !ifPictureClose
						&& !ifComputerClose && !ifDrawer1 && !ifWantToExit) {
					ifDrawer2 = true;
				}
				if (ifCode1) {
					...
					if (cable.ifChose(e) && !ifCablePicked && !ifWantToExit) {
						ifCablePicked = true;
				...
				if (e.getX() >= 195 && e.getX() <= 435 && e.getY() >= 570 && e.getY() <= 640 && !ifWantToExit) {
					message.setText("It's locked.");
					se.setFile(lockS);
					se.play();
					try {
